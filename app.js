...
```
  ...
  ${itemsTable(state.orderItems, true, "orders")}
...
// BUG FIX 1: Carton No auto-increment now correctly counts existing cartons
// for the selected order from the server, not just local cartonItems.
function getNextCartonNo(orderId) {

if (!orderId) return “1”;

const existing = state.cartons.filter(c =>
...
const orderOptions = state.orders.map(o => `<option value="${o.id}" ${state.selectedOrder && String(state.selectedOrder.id) === String(o.id) ? "selected" : ""} > #${o.id} — ${o.party_name} </option>`).join(””);
...
```
  ...
  ${itemsTable(state.cartonItems, true, "packing")}
...
// BUG FIX 1 applied: use getNextCartonNo for correct auto-increment
state.cartonNo = getNextCartonNo(state.selectedOrder.id);
...
// BUG FIX 2: itemsTable now receives explicit `context` parameter so
// deleteItem knows which list to modify — no more relying on state.tab,
// which breaks when table is rendered inside QC details (tab is “qc”
// but items are carton items, not order items).
// BUG FIX 3: “Total” column in QC/read-only view now shows the correct
// total_cartons value from the item itself rather than calling
// getCurrentTotalCartons() which reads live cartonItems (always empty in QC).
function itemsTable(items, del, context) {
...
```
  ...
  ${(items || []).map((it, i) => {
    ...
    // BUG FIX 3: use item's own total_cartons when available,
    // fall back to live getCurrentTotalCartons() only for active packing.
    const totalCartons =
      it.total_cartons != null
        ? it.total_cartons
...
// BUG FIX 2: deleteItem now uses the explicit context argument instead of
// state.tab so it always modifies the right list.
function deleteItem(i, context) {

const ctx = context || state.tab;
